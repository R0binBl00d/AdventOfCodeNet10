  ...
  internal class Part_1_2025_Day_03 : Days
  {
    ...
    */
    ...
    public override string Execute()
    {
      ...
      {
        #region takes the highest two distinct digits in order and make it a number
        //var digits = (from c in line select c).Distinct().OrderByDescending(c => c);
        //pos1 = line.IndexOf(digits.First());
        //if (digits.Count() > 1)
        //{
        //  pos2 = line.IndexOf(digits.ElementAt(1));
        //}
        //else
        //{
        //  pos2 = line.IndexOf(digits.First(), pos1 + 1); // Second occurrence
        //}

        //if (pos1 < pos2)
        //{
        //  result = line[pos1].ToString() + line[pos2].ToString();
        //}
        //else
        //{
        //  result = line[pos2].ToString() + line[pos1].ToString();
        //}

        //totalCount += long.Parse(result);
        #endregion

        var digits = (from c in line select c).Distinct().OrderByDescending(c => c);
        pos1 = line.IndexOf(digits.First()); // highest digit

        if(pos1 != line.Length-1) // there is substring behind the highest digit
        {
          var secondaryDigit = (from c in line[(pos1 + 1)..] select c).Distinct().OrderByDescending(c => c);
          // take the highest digit from the substring behind the highest digit
          result = line[pos1].ToString() + secondaryDigit.First().ToString();
        }
        else // the highest digit is at the end of the line
        {
          pos1 = line.IndexOf(digits.ElementAt(1)); // Second highest occurrence; 
          var secondaryDigit = (from c in line[(pos1 + 1)..] select c).Distinct().OrderByDescending(c => c);
          // take the highest digit from the substring behind the highest digit
          result = line[pos1].ToString() + secondaryDigit.First().ToString();
        }
