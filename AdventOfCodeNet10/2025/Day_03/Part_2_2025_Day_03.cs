...
namespace AdventOfCodeNet10._2025.Day_03
{
  internal class Part_2_2025_Day_03 : Days
  {
    ...
    */
    ...
    {
      string result = "";
      UInt128 totalCount = 0;

      foreach (var line in Lines)
      {
        List<(int pos, int count)> positionDistribution = new List<(int pos, int count)>();
        List<int> finalPositions = new List<int>(12);

        var digits = (from c in line select c).Distinct().OrderByDescending(c => c);

        int digitCount = 0;
        foreach (var digit in digits)
        {
          int pos = line.Length;
          (int digit, int count) item = new ValueTuple<int, int>(int.Parse(digit.ToString()), 0);

          while (line.LastIndexOf(digit, pos - 1) >= 0)
          {
            pos = line.LastIndexOf(digit,  pos - 1);
            finalPositions.Add(pos);
            item.count++;
            digitCount++;
            if (pos < 2) break;
          }
          positionDistribution.Add(item);
        }
        ...
        foreach (var pos in finalPositions)
        {
          if (Number.Length < 12)
          {
            // if we found a high digit left of the number we already have
            if (pos < leftMostPosition)
            {
              //check if the digit is smaller and we already skipped positions
              //if we skipped positions and the digit is smaller, fill the first skipped position on the left
              if (line[pos] < line[leftMostPosition])
              {
                // I don't want to make the number smaller !!
                // form       234234234234278
                // it goes                  8   
                // it goes                 78   
                // it goes               4 78   
                // it goes            4  4 78   
                // it goes         4  4  4 78   
                // it goes      4  4  4  4 78   
                // it goes      4  4  4 34 78   fill in the blanks from right to left while there is still a bigger digit left of if
                // it goes      4  4 34 34 78
                // it goes      4 34 34 34 78
                
                // because     34 34 34 34 78   SKIP !! would make the 12-digit number smaller, so instead I want to skip this one
                
                // we're still  4 34 34 34 78   
                // and use next 4 34 34 34278   fill in the blanks from right to left while there is still a bigger digit left of if
            ...
            else
            {
              ...
              //leftMostPosition = singleFinalPosition;
          }
          else break;
        }

        //totalCount += UInt64.Parse(sb.ToString());
        //sb.Clear();
      }
