  ...
  internal class Part_2_2025_Day_05 : Days
  {
    ...
    */
    ...
    public override string Execute()
    {
      ...
      var mergedRanges = new Stack<Range>();

      // sort the ranges by their start value
      freshRanges.Sort((a, b) => a.start.CompareTo(b.start));
      
      // add the first range to the merged list
      mergedRanges.Push(freshRanges[0]);

      // go through each range (from 2nd->Index 1) and merge if necessary
      for (int i = 1; i < freshRanges.Count; i++)
      {
        ...
        {
          mergedRanges.Pop(); // remove the last merged range to replace it with the merged one
          mergedRanges.Push((previousRange.start, Math.Max(previousRange.end, currentRange.end)));
          //mergedRanges[^1] = (lastMergedRange.Item1, Math.Max(lastMergedRange.Item2, currentRange.Item2));
        }
        // no overlap, add the current range to merged list
        else
        {
