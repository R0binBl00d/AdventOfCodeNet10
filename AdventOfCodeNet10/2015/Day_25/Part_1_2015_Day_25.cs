namespace AdventOfCodeNet10._2015.Day_25
{
  internal class Part_1_2015_Day_25 : Days
  {
    ...
    message asking you to enter a code
    ...
    "That model went out of support six minutes ago, and we just finished shredding
    all of the manuals.
    ...
    corner.
    The codes are filled in by diagonals: starting with the first row with an empty
    first box,
    ...
    previous one,
    multiplying it by 252533, and then keeping the remainder from dividing that
    value by 33554393.
    
    So, to find the second code (which ends up in row 2, column 1), start with the
    previous value, 20151125.
    ...
    "that's not even all of the first few numbers; for example, you're missing the
    one at 7,1 that would come before 6,2.
    ...
    */
    ...
    public override string Execute()
    {
      ...
      long maxCalculation = long.MaxValue; // 2947 * 3029;
      long finalCode = 0;
      int maxRow = 1;

      Point location = new Point(1,1);

      foreach (long l in CreateNumbers(20151125, maxCalculation))
      {
        if (location.X == 3029 && location.Y == 2947)
        {
          finalCode = l;
          break;
        }

        if (location.Y - 1 < 1) // start a new row
        {
          location.X = 1;
          location.Y = ++maxRow;
        }
      ...
      result = finalCode.ToString();
      return result;
    }

    private static IEnumerable<long> CreateNumbers(long first, long max)
    {
      long previousValue = first;
      yield return first;

      for (long i=1; i<max; i++)
      {
        long tmp = (previousValue * 252533) % 33554393;
        yield return tmp;
        previousValue = tmp;
      }
    }
