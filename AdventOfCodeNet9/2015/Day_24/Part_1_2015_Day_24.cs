namespace AdventOfCodeNet10._2015.Day_24
{
  internal class Part_1_2015_Day_24 : Days
  {
    ...
    and every package has to fit. The first group goes in the passenger compartment
    of the sleigh,
    ...
    packages as possible so that Santa has some legroom left over.
    ...
    packages such that the fewest possible are in the first group,
    you need to choose the way where the first group has the smallest quantum
    ...
    through 11.
    For this situation, some of the unique first groups, their quantum
    entanglements,
    ...
    the configuration with only two packages, 11 9,
    in the passenger compartment gives Santa the most legroom and wins.
    In this situation, the quantum entanglement for the ideal configuration is
    therefore 99.
    ...
    */
    ...
    {
      // Read the input from file and convert it to numbers
      var input = Lines.Select(line => int.Parse(line)).ToList();
      var totalSum = input.Sum();
      var groupWeight = totalSum / 3;

      var validPackages = new List<IEnumerable<int>>();

      // Find valid combinations
      for (int i = 1; validPackages.Count == 0; i++)
      {
        foreach (var combination in Combinatorics.Combinations(input, i))
        {
          if (combination.Sum() == groupWeight)
          {
            validPackages.Add(combination);
          }
        }
      }

      // Calculate the result
      var result = validPackages.Select(pkg => pkg.Aggregate(1L, (acc, x) => acc * x)).Min();
    ...
    public override string Execute()
    {
      ...
      {
        packages.Add(Int32.Parse(line));
      }

      var sum = packages.Sum();
      groupWeight = sum / 3; //512

      var tempList = new List<int>(packages.Count);
      for (int i = packages.Count - 1; i >= 0; i--)
      {
        tempList.Add(packages[i]);
      }
      Calculate += OnCalculate;

      CreateSequenceWithGroupWeight(new List<int>(), tempList, packages);

      return result;
    }

    private int groupWeight;
    public List<int[]> ResultsIntList = new List<int[]>();

    private void OnCalculate(List<int> combination, object itemtoworkon)
    {
      ResultsIntList.Add(combination.ToArray());
    }

    public event DoSomethingWithTheList Calculate;

    private int itemcount = Int32.MaxValue;

    private void CreateSequenceWithGroupWeight(List<int> sequence, List<int> modify, object itemToWorkOn)
    {
      if (sequence.Count > itemcount) return;

      int seqSum = sequence.Sum();
      if (seqSum > groupWeight) return;
      else if(seqSum == groupWeight)
      {
      ...
      for (int i = 0; i < modify.Count; i++)
      {
        ...
        var lessCombinations = new List<int>(modify);
        newSequence.Add(lessCombinations[i]);
        lessCombinations.RemoveAt(i);
        CreateSequenceWithGroupWeight(newSequence, lessCombinations, itemToWorkOn);
      }
    }

    public delegate void DoSomethingWithTheList(List<int> combination, object itemToWorkOn);
    */
    ...
    public class Combinatorics
    {
      public static IEnumerable<IEnumerable<T>> Combinations<T>(IEnumerable<T> list, int length)
      {
        ...
        else
        {
          ...
          {
            IEnumerable<T> remainingItems = AllExceptFirst(startingElementIndex, list);

            foreach (IEnumerable<T> combinationOfRest in Combinations(remainingItems, length - 1))
            {
              yield return Concat(startingElement, combinationOfRest);
            }
      ...
      private static IEnumerable<T> AllExceptFirst<T>(int index, IEnumerable<T> source)
      {
        return source.Skip(index + 1);
      }

      private static IEnumerable<T> Concat<T>(T first, IEnumerable<T> second)
      {
        yield return first;
