namespace AdventOfCodeNet9._2023.Day_02
{
    ...
    */
    ...
    public override string Execute()
    {
      ...
      foreach (var line in Lines)
      {
        ...
        // gameindex
        int iGameIndex = Int32.Parse(chunks[0].Split(' ')[1]);

        bool bGamePossible = true;

        for(int i = 1; i < chunks.Length; i++) // games for given gameIndex
        {
          cubeGame.Clear();
          // cubes
          var chunkCubes = chunks[i].Split(new char[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries);
          for(int j = 0; j < chunkCubes.Length; j+=2)
          {
            switch(chunkCubes[j+1]) 
            {
              case "red":
                cubeGame.RED = Int32.Parse(chunkCubes[j]);
                break;
              case "green":
                cubeGame.GREEN = Int32.Parse(chunkCubes[j]);
                break;
              case "blue":
                cubeGame.BLUE = Int32.Parse(chunkCubes[j]);
                break;
            }
          }
          bGamePossible &= cubeGame.IsValid();
        }

        if(bGamePossible)
        {
          sumIndexes += iGameIndex;
        }
      }
      result = sumIndexes.ToString();
      return result;
    }
    ...
    private class CubeGame
    {
      ...
      public CubeGame(int maxRed, int maxGreen, int maxBlue)
      {
        mRED = maxRed;
        mGREEN = maxGreen;
        mBlue = maxBlue;
