...
namespace AdventOfCodeNet8._2023.Day_04
{
  internal class Part_2 : Days
  {
    ...
    Your fourteen instances of card 5 (one original and thirteen copies) have no matching numbers and win 
    no more cards.
    ...
    */
    /// </summary>
    /// <returns>
    /// 14752554 is too low (had >= in Line 156)
    /// 14814534
    /// </returns>
    public override string Execute()
    {
      ...
      int maxTickets = Lines.Count;
      Dictionary<int, int> tickets = new Dictionary<int, int>(); // ID & count
      ...
      foreach (var line in Lines)
      {
        ...
        int ticketID = Int32.Parse(chunks[0].Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)[1].Trim());
        ...
        {
          tickets.Add(ticketID, 1);
        }

        foreach (var wn in chunks[1].Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries))
        {
          winningNumbers.Add(int.Parse(wn.Trim()));
        }
        foreach (var mn in chunks[2].Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries))
        {
          myNumbers.Add(int.Parse(mn.Trim()));
        }
        ...
        for (int t = 0; t < tickets[ticketID]; t++)
        {
          ...
          {
            if (newTicketID > maxTickets) continue;

            if (tickets.ContainsKey(newTicketID))
            {
              tickets[newTicketID]++;
            }
            else
            {
              tickets.Add(newTicketID, 1);
            }
            newTicketID++;
          }
        }
      ...
      foreach(var tkey in tickets.Keys) 
      {
        totalScore += tickets[tkey];
      }
