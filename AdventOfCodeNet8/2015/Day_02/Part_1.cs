...
namespace AdventOfCodeNet8._2015.Day_02
{
  internal class Part_1 : Days
  {
    ...
    */
    ...
    public override string Execute()
    {
      ...
      {
        var sideLength = s.Split('x');

        int l = Int32.Parse(sideLength[0]);
        int w = Int32.Parse(sideLength[1]);
        int h = Int32.Parse(sideLength[2]);

        int side_lw = l * w;
        int side_wh = w * h;
        int side_hl = h * l;

        int min = Math.Min(side_lw, Math.Min(side_wh, side_hl));

        total += 2 * side_lw + 2 * side_wh + 2 * side_hl + min;
      }
